...
class DynamicStorageAdapter(nn.Module):
    def __init__(self, embed_dim: int, n_storage_tokens: int):
        super().__init__()
        self.n = n_storage_tokens
        self.d = embed_dim
        self.net = nn.Sequential(
            nn.Linear(embed_dim, embed_dim // 2),
            nn.GELU(),
            nn.Linear(embed_dim // 2, n_storage_tokens * embed_dim),
        )

    def forward(self, patch_embeds: Tensor) -> Tensor:
        g = patch_embeds.mean(dim=1)              # [B, C]
        return self.net(g).reshape(-1, self.n, self.d)  # [B, n, C]


class StorageGuidedLoRAScaleGenerator(nn.Module):
    """Generates per-token per-block LoRA scales (token-level adaptive scaling)."""
    ...
    def forward(self, patch_tokens: Tensor) -> Tensor:
        # patch_tokens: [B, N, C] → [B, N, n_blocks]
...
class DinoVisionTransformer(nn.Module):
    ...
    def enable_dynamic_lora(self, rank: int = 8) -> None:
        for blk in self.blocks:
            blk.attn.enable_lora(rank)
            blk.mlp.enable_lora(rank)
        self.lora_scale_gen = StorageGuidedLoRAScaleGenerator(self.embed_dim, self.n_blocks)
    ...
    def prepare_tokens_with_masks(self, x: Tensor, masks=None) -> Tuple[Tensor, Tuple[int]]:
        ...
        if self.n_storage_tokens > 0:
            if self.storage_adapter is not None:
                delta = self.storage_adapter(x)
                storage_tokens = self.storage_tokens + delta
            else:
    ...
    def _get_intermediate_layers_not_chunked(self, x: Tensor, n: int = 1) -> List[Tensor]:
        ...
        # Generate per-block LoRA scales from initial patch context
        lora_scales = None
        if hasattr(self, 'lora_scale_gen'):
            n_prefix = 1 + self.n_storage_tokens
            patch_scales = self.lora_scale_gen(x[:, n_prefix:])  # [B, N_patch, n_blocks]
            B = x.shape[0]
            prefix_scales = torch.ones(B, n_prefix, self.lora_scale_gen.n_blocks,
                                       device=x.device, dtype=patch_scales.dtype)
            lora_scales = torch.cat([prefix_scales, patch_scales], dim=1)  # [B, N_total, n_blocks]
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if lora_scales is not None:
                blk.attn._lora_scale = lora_scales[:, :, i]  # [B, N_total]
                blk.mlp._lora_scale = lora_scales[:, :, i]   # [B, N_total]
            x = blk(x, rope_sincos)
