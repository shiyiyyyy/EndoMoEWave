...
class DepthAnythingV2(nn.Module):
    def __init__(
        ...
        if self.pretrained.n_storage_tokens > 0:
            self.pretrained.storage_adapter = DynamicStorageAdapter(
                self.pretrained.embed_dim, self.pretrained.n_storage_tokens
            )

        self.pretrained.enable_dynamic_lora(rank=4)
